...
function findLongestWord(sentence) {
  const arrStr = sentence.split(' ').sort((a, b) => b.length - a.length);
  return arrStr[0];
}
...
function reverseWords(str) {
  const revString = str.split(' ').map((e) => e.split('').reverse().join(''));
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-zA-Z]/g, (char) => shifted[alphabet.indexOf(char)]);
}
